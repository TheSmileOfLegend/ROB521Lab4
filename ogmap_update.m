function [og, imml] = ogmap_update(og, x, phi_m, r_m, r_max, r_min, alpha, beta, p_occ, p_free, mode)
...
if ((mode == 1) || (mode == 2))
    ...
    invmod = inverse_scanner_window(M, N, x, phi_m, r_m, r_max, r_min, alpha, ...
        beta, p_occ, p_free);
    ...
    for i = 1:length(phi_m)
        ...
        invmod = inverse_scanner_bres(M, N, x(1), x(2), phi_m(i)+x(3), ...
            r_m(i), r_max, r_min, p_occ, p_free);
